...
public class Hub {
	 ...
	 * method for more information on resetting the output stream. The default
	 * value is false.
	 */
	...
	synchronized private void clientDisconnected(int playerID) {
		if (playerConnections.containsKey(playerID)) {
			...
			System.out
					.println("Connection with client number " + playerID + " closed by DisconnectMessage from client.");
		}
	}
	...
	private class ConnectionToClient { // Handles communication with one client.
		...
		private class SendThread extends Thread {
			public void run() {
				...
				try {
					while (!closed) { // Get messages from outgoingMessages
										// queue and send them.
						try {
							...
							if (message instanceof ResetSignal)
								...
								out.flush();
								if (message instanceof DisconnectMessage) // A
																			// signal
																			// to
																			// close
																			// the
