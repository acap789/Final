 ...
 * netgame Hub. The client is used for sending messages to the Hub (and through
 * the Hub possibly to other clients). It also receives messages from the Hub.
 * An application must define a subclass of this abstract class in order to
 * create players. Each player in the game will have an associated Client
 * object, which will handle details of network communication with the Hub.
 * <p>
 * At a minimum, the abstract method messageReceived(Object) must be defined to
 * say how the Client should respond when a message is received. Subclasses
 * might also define the closedByError(), serverShutdown(), playerConnected(),
 * and playerDisconnected() methods.
 * <p>
 * A client provides methods send(Object) and disconnect() for sending a message
 * to the Hub and for closing down the connection. Any non-null object that
 * implements the Serializable interface can be sent as a message. Note that an
 * ObjectOutputStream is used for sending messages. If the same object is to be
 * sent more than once, with changes between transmissions, then the
 * resetOutput() method should be called between transmissions (or the autoreset
 * property should be set to true).
 * <p>
 * A client has a unique ID number that is assigned to the client when it
 * connects to the hub. The ID can retrieved by calling the getID() method. The
 * protected variable connectedPlayerIDs contains the ID numbers of all clients
 * currently connected to the hub, including this one.
 */
abstract public class Client {

	/**
	 * A list of the ID numbers of all clients who are currently connected to
	 * the hub. This list is set each time this client is notified that a client
	 * has connected to or disconnected from the hub.
	 */
	...
	/**
	 * Constructor opens a connection to a Hub. This constructor will block
	 ...
	 *            requests.
	 * @throws IOException
	 *             if any I/O exception occurs while trying to connect.
	 */
	...
	/**
	 * This method is called when a message is received from the Hub. Concrete
	 * subclasses of this class must override this method to say how to respond
	 * to messages. Messages can be arbitrary Serializable objects.
	 */
	abstract protected void messageReceived(Object message);

	/**
	 * This method is called whenever this client is notified that a client has
	 * connected to the hub. (Note that it is called when this client connects,
	 * so this method will be called just after the connection has been
	 * established.) The list of all connected players, including the new one,
	 * is in the protected variable connectedPlayerIDs. The method in this class
	 * does nothing.
	 * 
	 * @param newPlayerID
	 *            the ID number of the player who has connected.
	 */
	protected void playerConnected(int newPlayerID) {
	}

	/**
	 * This method is called when this client is notified that a client has
	 * disconnected from the hub. (Note that it IS NOT called when this client
	 * disconnects.) The list of all connected players is in the protected
	 * variable connectedPlayerIDs. The method in this class does nothing.
	 * 
	 * @param departingPlayerID
	 *            the ID number of the player who has just disconnected.
	 */
	 ...
	 * 
	 * @param in
	 *            a stream from which messages from the hub can be read.
	 * @param out
	 *            a stream to which messages to the hub can be written. After
	 ...
	 * 
	 * @param message
	 *            A non-null object representing the message. This object must
	 *            implement the Serializable interface.
	 * @throws IllegalArgumentException
	 *             if message is null or is not Serializable.
	 * @throws IllegalStateException
	 *             if the connection has already been closed, either by the
	 *             disconnect() method, because the Hub has shut down, or
	 ...
	 * client among all clients which have connected to the hub. ID numbers are
	 * always assigned in the order 1, 2, 3, 4... There can be gaps in the
	...
	/**
	 * Resets the output stream, after any messages currently in the output
	...
	public void resetOutput() {
		connection.send(new ResetSignal()); // A ResetSignal in the output
											// stream is seen as a signal to
											// reset
	}

	/**
	 * If the autoreset property is set to true, then the output stream will be
	...
	private final ConnectionToHub connection; // Represents the network
	...
	private class ConnectionToHub {
		...
		private final int id_number; // The ID of this client, assigned by the
										// hub.
		private final Socket socket; // The socket that is connected to the Hub.
		private final ObjectInputStream in; // A stream for sending messages to
											// the Hub.
		private final ObjectOutputStream out; // A stream for receiving messages
												// from the Hub.
		private final SendThread sendThread; // The thread that sends messages
												// to the Hub.
		private final ReceiveThread receiveThread; // The thread that receives
													// messages from the Hub.

		private final LinkedBlockingQueue<Object> outgoingMessages; // Queue of
																	// messages
																	// waiting
																	// to be
																	// transmitted.

		private volatile boolean closed; // This is set to true when the
											// connection is closing.
											// For one thing, this will prevent
											// errors from being
											// reported when exceptions are
											// generated because the
											// connection is being closed in the
											// normal way.

		/**
		 * Constructor opens the connection and sends the string "Hello Hub" to
		 * the hub. The hub responds with an object of type Integer representing
		 * the ID number of the client. The extraHandshake() method is then
		 * called to do any other required startup communication. Finally,
		 * threads are created to handle sending and receiving messages.
		 */
		ConnectionToHub(String host, int port) throws IOException {
			...
			extraHandshake(in, out); // Will throw an IOException if handshake
										// doesn't succeed.
		...
		/**
		 * This method is called to close the connection. It can be called from
		 * outside this class, and it is also used internally for closing the
		 * connection.
		 */
		void close() {
			closed = true;
		 ...
		 * 
		 * @param message
		 *            the message, which must be a Serializable object.
		 */
